...
solução semelhante a figura \ref{fig:exemplopd1}.
...
Sobre a análise feita por computadores, Rowe \cite{rowe09:levels}
acrescenta:
...
o conjunto de dados e estruturas geradas pelo músico e pelo sistema. Nesses dados não há como separar
a influência do sistema no músico. O resultado da análise desses dados pode revelar
...
para análise de música eletroacústica. Nesse sentido podemos apontar os artigos
...
de ``instrumento aumentado'' ou ``\textit{hyperinstrument}'' como definido por Machover\cite{hyperinstrumento}. 
...
Tradução\footnote{Detalhes de tradução: prefei manter os termos técnicos originais.} dos primeiros 3 capítulos do documento
...
de abstrações (e.g. Uma ``biblioteca de abstrações'' tal qual o presente projeto
...
{\em Library}- Os arquivos devem seguir um sistema dependente de convenções de nomes:
...
Diferente dos {\em externals}, {\em libraries} podem ser importadas pelo Pd com operações especiais.
Depois que uma {\em library} tenha sido importada,
todos {\em externals} incluídos são carregados na memória e ficam disponíveis como objetos.

Pd possui dois modos para importar {\em libraries}:

\begin{itemize}
\item via opção de linha de comando ``{\tt -lib my-lib}''
\item criando um objeto ``{\tt my-lib}''
\end{itemize}

O primeiro método carrega uma {\em library quando Pd é iniciado.
Esse método é preferencialmente usado para {\em libraries} que contém muitos {\em externals}.

O outro método deve ser usado para {\em libraries} que contém exatamente um
{\em external} com o mesmo nome.

Pd checa primeiro, se uma classe chamada ``my-lib'' já foi carregada.
Se esse não for o caso\footnote{
Se uma classe ``my-lib'' já é existente, um objeto ``my-lib'' será inicializado e o procedimento
 será concluído. 
Assim, nenhuma {\em library} foi carregada.
Portanto nenhuma {\em library} que tenha o mesmo nome que uma classe já usada como, por exemplo ``abs'',
pode ser carregada.}, é feita uma busca em todos \textit{paths}\footnote{Os Paths ou caminhos de diretórios são pré-definidos pelo usuário.}
por um arquivo chamado ``{\tt my-lib.pd-linux}''\footnote{ou outra extensão dependente do sistema que se está trabalhando.}.
Se tal arquivo é encontrado, todos {\em externals} incluídos são carregados na memória
através de uma chamada da rotina \verb+my-lib-setup()+.

Após o carregamento, uma classe ``my-lib'' é (novamente) buscada como um (recentemente carregado) {\em external}.
Em caso afirmativo, uma instância dessa classe é criada, ou senão a inicialização falha e um erro
é impresso no console.
De qualquer maneira, todas classes de {\em externals} declarados na  {\em library} são carregados.


\subsection{meu primeiro external: {\tt alo\_mundo}}
Usualmente a primeira abordagem para se aprender uma linguagem de programação é algum tipo
de aplicação ``alô mundo''.

Em nosso caso, uma classe de objeto deve ser criada, que imprima a linha ``alo mundo!!''
como erro padrão toda vez que receba uma mensagem de `bang''.


\subsubsection{a interface com o Pd}
Para escrever um external de Pd é necessário uma interface bem definida.
Isso é fornecido no arquivo-cabeçalho ``m-pd.h''

\begin{verbatim}
#include "m-pd.h"
\end{verbatim}

\subsubsection{uma classe e seu espaço de dados}
primeiro uma nova classe deve ser preparada e o espaço de dados para essa classe deve ser
definido.
...
\verb+helloworld-class+ será um ponteiro para a nova classe.

A estrutura \verb+t-helloworld+ (do tipo \verb+-helloworld+) é o espaço
de dados da classe.

Um elemento absolutamente necessário do espaço de dados é uma variável do
tipo \verb+t-object+, que é usada para armazenar propriedades internas do objeto
como a apresentação gráfica do objeto ou dados sobre entradas e saídas gráficas.

\verb+t-object+ deve ser a primeira especificação da estrutura.

Um simples objeto ``alô mundo'' não necessita de variáveis,
portanto a estrutura fica vazia além de \verb+t-object+.


\subsubsection{method space}
Além do espaço de dados, uma classe necessita de uma sequência de manipuladores
(métodos) para manipular o fluxo de dados à passar pelo objeto.
