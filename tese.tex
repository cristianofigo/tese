...
O eterno retorno não pode significar o retorno do Idêntico, poisele supõe, 
...
No primeiro tipo de repetição, as referências mais comuns sãoas repetições de frases, 
de notas ou de sonoridadesespecíficas dentro de um enunciado musical. Esses elementos podem 
ou não estar associados ao segundo tipo de repetição;a repetição de emoções, de associações e 
sentimentos queafloram em forma de lembranças quando evocados por um fatosonoro experienciado 
anteriormente.
...
polares.Podemos converter as coordenadas do sistema polar para cartesiano tirando uma raiz quadrada
da soma do valor real e imaginário elevados a própria potência. Isso pode ser feito no Pd com os
objetos [*\texttildelow] abaixo de [rfft\texttildelow] realizando a potência e enviando o resultado 
...
The left inlet is the incoming audio signal, the middle control input sets center frequency and the rigth input sets "Q".
...
%% pegar REFERENCIAS do caesar e rowe 
...
uma situação de performance com dados MIDI onde não se dispõe de um controlador externo. O objetivo
é transformar o teclado alfa-numérico do computador em um controlador MIDI sem o controle de velocity
de cada nota. Cada tecla é mapeada com a abstração [kbkey2] o que permite que se defina como quiser
as relações das teclas com uma respectiva nota MIDI.

Na figura XXX podemos ver o panorama interno da abstração [sinc-teclado], onde podemos notar
que cada tecla envia uma mensagem MIDI por [noteout] para o canal 2. 
Na figura XXX vemos a distribuição das instâncias de [kbkey2] mapeando
cada tecla. Na figura XXX vemos o detalhamento dos argumentos de criação de [kbkey2], que além de mapear
...
A maioria das linguagens de programação, pacotes estatísticos ou planilhas de cálculo possuem um 
gerador de números aleatórios, que gera a partir de uma distribuição uniforme, com valores entre 0 e 1. 
Esse número é chamado de pseudo-aleatório, porque é possível repetir a mesma sequência a partir de uma 
mesma semente (valor inteiro).

Em estatística e probabilidade, a distribuição uniforme é a distribuição de probabilidades contínua 
mais simples de conceituar: a probabilidade de se gerar qualquer ponto em um intervalo contido no 
espaço amostral é proporcional ao tamanho do intervalo.

Random numbers are generated via a complex equation which produces a pseudo-predictable sequence of numbers. 
Each [random] object's equation is provided with a "seed" which is really just one of the variables in the 
equation which produces the sequence. The "seed" is generated by Pd based on specific parameters in each 
patch which contains a [random] object. If more than one [random] objects are contained within a single 
patch, they each get a different "seed".

However, "seeds" can be inputted into [random] by sending a message, like below:

Seeds are kept locally so that if two [random]s are seeded the same, they will have the same output - 
as demonstrated below. Furthermore, you can seed the same one twice to repeat the output.
...
O Pd-extended traz uma implementação de boids na forma de um objeto que recebe mensagens de parâmetros
de comportamento global, parâmetros do centro de atração e calcula as novas posições XYZ de cada
boid. Foi feita uma abstração que representa cada boid que pode ser vista na figura BOIDZZ.

Nesse patch podemos observar que.


As mensagens de parâmetros de comportamento global podem ser vistas na figura GLOBAL. Onde vemos
parâmetros como ``atracttion'' que é um fator de maior atração do grupo de boids em direção
ao centro de atração que é definido pela mensagem ``atractioncenter'' (?)

No caso de uma aplicação musical desse algoritmo foi pensado num primeiro momento que as frequências
podem representar o eixo X e as amplitudes o eixo Y. 

O efeito musical é uma textura heterofônica, pois a nota da análise do aúdio do músico se torna
o centro de atração do grupo de boids.


%%fazer imagens: boidzz + mensagens globais + partitura de uma resultado com 4 vozes?

Outros algoritmos podem ser encontrados na biblioteca ``flatspace'' como chaos attractors:

attract1 base base3 gingerbreadman henon hopalong ikeda latoocarfian latoomutalpha latoomutbeta 
latoomutgamma lorenz martin popcorn quadruptwo rossler standardmap
...
% sinc-alternate.pd
% sinc-analise-help.pd
% sinc-analise.pd
% sinc-audioanalise-help.pd
% sinc-audioanalise.pd
% sinc-audioin.pd
% sinc-buffer_temp.pd
% sinc-calc_melodico.pd
% sinc-calc_ritmo.pd
% sinc-cenario.pd
% sinc-cenarios.pd
% sinc-cenas.pd
...
% sinc-chordseq.pd
% sinc-chord-table.pd
% sinc-controle_teclado.pd
% sinc-dens_ritmica-help.pd
% sinc-dens_ritmica.pd
% sinc-escala-linear.pd
% sinc-estabilidade_ritmica.pd
% sinc-fx.pd
% sinc-gera_dinamica.pd
% sinc-gera_melodico.pd
% sinc-gera_melodicos.pd
% sinc-gera_ritmo.pd
% sinc-gera_sintese-help.pd
% sinc-gera_sintese.pd
% sinc-gravador.pd
% sinc-harmonia.pd
% sinc-harmonia-table.pd
% sinc-idtimbre.pd
% sinc-looper4.pd
% sinc-looper-help.pd
% sinc-looper.pd
% sinc-looper-pedal.pd
% sinc-loop-help.pd
% sinc-loop_master.pd
% sinc-loop_slave.pd
% sinc-mandanotacao.pd
% sinc-midiin-help.pd
% sinc-midiin.pd
% sinc-midi_osc1-help.pd
% sinc-midi_osc1.pd
% sinc-midiout.pd
% sinc-mixer-help.pd
% sinc-mixer.pd
...
% sinc-pauta1.pd
% sinc-pauta.pd
% sinc-permeabilidade.pd
% sinc-prob.pd
% sinc-recebenotacao.pd
% sinc-sample.pd
% sinc-slicer.pd
% sinc-teclado.pd
% sinctest_delay1.pd
% sinctest_delayspecial2.pd
% sinctest_delayspecial.pd
% sinctest_fm1.pd
% sinctest_fm2.pd
% sinctest_fmdelay2.pd
% sinctest_fmdelay.pd
% sinctest_teclado.pd
