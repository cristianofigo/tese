...
% nesse paragrafo devo separar as idéias e aprofundar cada idéia

%% idéia 1: a ferramenta tem uma estética embutida - conduzir essa idéia
% até a revelação de que a decisão por um algoritmo conduz o resultado 
% estético


%% idéia 2: o simulacro computacional exato da relação musical entre dois músicos:
%% - o mito da "verdade estrutural" revelada a partir da análise da performance musical,
%% - o mito da descoberta do comportamento musical,
%% concluir dizendo para que serve artisticamente essa pesquisa voltada ao simulacro computacional
...
%% aqui explorar essa idéia de "imersão" para concluir como esse desejo de imersão está
%% presente na concepção desse trabalho
...
% \subsection{Poética de SInCoPA}
% \label{sec:poetica:sincopa}
% 
% 
% 
% A realização deste trabalho reflete a conexão entre
% diversos desejos presentes na cultura contemporânea que
% se materializam em uma prática musical através de um conjunto de 
% programas para interatividade musical. Nesse sentido, 
% a poética implícita na criação de SInCoPA é o resultado
% de uma confluência desses diversos vetores estéticos.
% 
% Nesta seção serão expostos os vetores que impulsionaram
% o desejo de realização deste trabalho e que fazem parte 
% de sua matriz estrutural. 
% 
% %% 5) noise e cultura do ruído - rock, punk
% %% 2) composição do timbre
% 
% O primeiro vetor é a influência sonora do que chamo aqui
% de cultura do ruído, um fenômeno observado mais nitidamente
% a partir do século XX. Penso que a partir do advento do
% manifesto futurista estabeleceu-se uma tradição da subversão
% sonora que evoluiu de maneira quase independente da academia e 
% do mercado musical, tornando-se uma espécie de celeiro musical fecundo e marginal.
% 
% Nesta tradição encontram-se os pioneiros da música eletrônica dos
% anos 50, os compositores de música experimental dos anos 60, o movimento punk dos
% anos 70 e suas formas mais radicais como o grindcore dos anos 80 e por fim
% o movimento noise dos anos 90. Em minha formação pessoal como músico e ouvinte
% considero como principais influências os trabalhos de Pierre Schaeffer,
% John Cage, Merzbow, bandas como Napalm Death, Carcass e Sonic Youth.
% Além dos trabalhos inspiradores de Walter Smetak, George Lewis e Miller Puckette.
% 
% 
% Como que SInCoPA faz parte dessa tradição
% 
% %% 1) composição algorítmica
% 
% 
% 
% De certa maneira, uma composição musical além de ser uma peça de arte, também
% pode ser considerada um objeto técnico que pode conter regras e funções arraigadas
% em sua estrutura. No caso de SInCoPA essa relação com o conceito de objeto
% técnico é mais explícita, por se tratar de um conjunto de aplicativos musicais.
% Essa aproximação revela um vetor 
% 
% 
% 
% Do ponto de vista da técnica composicional SInCoPA
% 
% SInCoPA surge do desejo de criar música usando sons capturados e processados, combinados
% com diversas técnicas de síntese sonora no sentido de explorar os limites
% da combinação timbrística, num contexto de instrumento aumentado interativo.
% Enquanto objeto técnico, o projeto de SInCoPA tem a ambição de ser uma realização
% que seja aberta e estimule a busca coletiva por uma simbiose sensível entre musicalidade e máquina.
% 
% 
% 
%  
% Um algoritmo é uma sequência não ambígua de instruções que é executada até que determinada condição se 
% verifique. Na composição algorítmica, o papel do computador pode variar desde uma calculadora
% especializada que ``resolve'' certas distribuições de parâmetros bem controlados até papéis mais radicais 
% realizando trechos musicais inteiros fora do controle do compositor. Segundo Roads:
% 
% Estruturas musicais organizadas em regras e funções são comuns à composição musical em diversos níveis. Um
% desejo de SInCoPA sempre foi
% 
% 
% \begin{quote}
...
% 
% 
% % Ainda que se possa generalizar
% % todos esses desejos é positivo afirmar que alguns desses desejos/vetores
% % podem ser entendidos como pertencentes à vertente técnica musical, enquanto que outros
% % pertencem ao universo da política e comportamento. 
% 
% 
% 
% 
...
% 
% 
% %% 3) conceito de instrumento aumentado ou instrumento inteligente
% %% 4) Práticas de cultura digital e conhecimento livre
% 
% %% 6) composição cognitiva (?)
% %% 7) code as art - programação enquanto composição
% %% 8) história do objeto técnico
% %% 9) composição como a criação de um mundo - criação de uma "cultura" - qual mundo quero criar
% %% 10)  
% 
% 
% 
% 
% De maneira geral, no conjunto dessas influências percebo esta pesquisa como 
% um movimento na direção de uma já tradicional quebra de paradigma da 
% atividade compositiva tradicional. Apesar desse desejo de rompimento,
% confesso que existe uma vontade maior de conciliação harmoniosa entre esses
% vetores e a prática composicional acadêmica.
% 
% 
% 
% 
...
\begin{figure}
\includegraphics[scale=.4]{espectromorfologia}
...
\begin{figure}
\includegraphics[scale=.6]{qjackctl}
...
\begin{figure}
\includegraphics[scale=.37]{sinc-audioanalise}
...
\begin{figure}
\includegraphics[scale=.37]{similaridade-interface}
...
\begin{figure}
\includegraphics[scale=.5]{prot5a}
\caption{comparação de arrays de durações}
\label{prot5a}
\end{figure} 
...
\begin{figure}
\includegraphics[scale=.45]{sinc-calc-ritmo}
...
\begin{figure}
\includegraphics[scale=.37]{ex-ritmo3}
...
\begin{figure}
\includegraphics[scale=.45]{prot5b}
...
\begin{figure}
\includegraphics[scale=.65]{prot5c}
...
\begin{figure}
\includegraphics[scale=.45]{audio-features}
\caption{timbreID - audio-features}
\label{audio-features}
\end{figure}

\begin{figure}
\includegraphics[scale=.45]{timbreid}
\caption{timbreID}
\label{timbreid}
\end{figure}

\begin{figure}
\includegraphics[scale=.5]{bfcc}
\caption{timbreID - bfcc}
\label{bfcc}
\end{figure}

\begin{figure}
\includegraphics[scale=.23]{pdescriptor}
...
\begin{figure}
\includegraphics[scale=.34]{audio2midi}
...
\begin{figure}
\includegraphics[scale=.55]{variacoes-contorno}
...
\begin{figure}
\includegraphics[scale=.5]{midi-boid}
...
\begin{figure}
\includegraphics[scale=.35]{harm5}
...
\begin{figure}
\includegraphics[scale=.4]{spectro}
