...
\bancaiiiNome{Lucas Robatto}
\bancaiiiTitulo{Doutor em Flauta}
\bancaiiiEstudo{Universidade de Washington, EUA}
\bancaiiiAtuacao{Universidade Federal da Bahia}
  
\bancaivNome{Jamary Olveira}
\bancaivTitulo{Doutor em Composição}
\bancaivEstudo{\textit{University of Texas at Austin}, EUA}
\bancaivAtuacao{Universidade Federal da Bahia}
  
\bancavNome{Jamary Olveira}
\bancavTitulo{Doutor em Composição}
\bancavEstudo{\textit{University of Texas at Austin}, EUA}
...
In recent years, several projects have been developed and adopted
...
in the field of computer music. Pd is a graphical language ...

Even allowing for different solutions to the composer, the Pd requires
the formal study of its grammar so that any language
programming. Many of the solutions presented by the community
are specific and hardly adaptable to general problems of
composition.

In this study we investigated the problem of creating a library
utilities that perform the interaction between the knowledge needed
for creating interactive music. The creation of this library allows
interconnecting hybrid solutions and various other general libraries
specific developed by the community. The use of the library
the composer provides a rapid prototyping in interactive music and
...
O sistema é construído com a linguagem \textit{Pure data} (\pd). 
...
% 
% pd2live is a digital music performance system, built in Pure Data (extended 0.42.5), designed 
% specifically to function with a polyphonic audio output (audio per string) of an augmented 
% electric guitar provided by bespoke audio break-out hardware. The system was designed in support 
% of my doctoral thesis (available shortly), providing a digital performance environment from which a
% n instrumentalist may interrogate instrumental percepts in relation to the creation of sophisticated 
% timbral and spatial digital audio signal processes.
% 
% On a fundamental level, the user may design and implement unique real-time signal processes per 
% string of the guitar, permitting the creation of dense polyphonic instrumental musical structures. 
% The (spectromorphological) design of each string may correspond to real-time instrumental figuration 
% executed by the performer.
% 
...
% unique performance gesture relationships within a physical (multi-channel) performance space.
...
sonora open-source e baseado na linguagem C. Disponível em: www.csounds.com}.
...
``orchestra'' e ``score''\footnote{Os termos ``orchestra'' e ``score'' 
no contexto do Csound podem ser traduzidos para o português como instrumentos e partitura}. 
...
é essencialmente uma sequência temporal, enquanto que a orchestra
...
O principal desenvolvedor inicial do Max, foi Miller Puckette
que em 1996 escreveu o Pd e disponibilizou na internet. 
...
of its performance, for example. \cite{rowe2004machine}


\footnote{Há um certo paradoxo no coração da 
transferência de musical
conhecimento para uma máquina. Temos que trabalham fortemente para fazer um computador
programar efectuar as análises necessárias de um estudante de música de calouro.
...
\begin{figure}
\includegraphics[scale=.7]{analise}
...
more flexible while being far less structured. \cite{rowe09:levels}

\footnote{A primeira onda de sistemas interativos de música baseado quase exclusivamente no Musical
Instrument Digital Interface (MIDI) padrão, uma representação simbólica da música modelado
estreitamente sobre o comportamento de um teclado de piano, bem como os conceitos tradicionais de notação de música.
A segunda onda toma como entrada sinais de áudio-primas, uma sub-representação simbólica de dados que está longe
mais flexível, sendo muito menos estruturado.}
\end{quotation}


\begin{figure}
\includegraphics[scale=.3]{rowe}
...
A descrição dos níveis de análise musical em simbólico e sub-simbólico, é apresentado por Leman, 
...
global structure based on this type of representation.\cite{leman89}

\footnote{Processamento de informação Resumo Subsymbolic pode fornecer novas formas de lidar com os aspectos
comportamento comunicativo que até agora têm sido muito difíceis de tratar em modelos simbólicos.
Parte de nossa análise é mostrar que as atividades musicais, como ouvir e compor empate em
grande em princípios dinâmicos do que é comumente chamado de percepção e / ou imaginação auditiva.
Argumenta-se que diferentes tipos de mecanismos de processamento subsymbolic informações podem ser exploradas
e aplicados a este campo como alternativas e / ou abordagens complementares aos modelos simbólicos.
...
é divorciada do formulário. Isto envolve uma mudança de representações proposicionais da música analógica
representações (onde o conteúdo está contido no formulário). Representações analógicas de música
não foram totalmente exploradas até agora. Argumentamos em favor de uma representação analógica musical
com base no formato Psicoacústica e propor este sistema como uma alternativa para a lógica de discurso
aproximar a representação musical. Um exemplo é dado de um sistema de auto-organização com um emergente
estrutura global com base neste tipo de representação.}
\end{quotation}
...
real-time kernel patches but I preferred to use off-the-shelf linux).  \cite{Puckette_patchfor}

\footnote{Eu estive trabalhando em um projeto de longo prazo para projetar uma vez personalizado
instrumento de música do computador para tentar trazer para fora e enfrentar algumas das dificuldades encontradas pelos
músicos tentando usar computadores em performance ao vivo. O instrumento é baseado num eléctrico compacto
violão (Steinberger / Gibson), com um captador adicional de seis cordas separadas (Roland). Não encontrando um
barato e compacto amplificador de 6 canais no mercado, eu projetei e construí um muito bruto.
Esta é conectado a um computador usando um multicanal interface PCI (Midiman). Um patch de Pd, correndo em
linux, em seguida, executa uma série de transformações interessantes sobre os seis sinais de áudio, e mistura-los
para estéreo para saída.

Isto é completamente diferente do padrão `` sintetizadores de guitarra ", que passo rastrear as cordas para conduzir
sintetizadores. Tais instrumentos cometa muitos erros audíveis, e eles também sofrem com a latência adicionada
o vem do rastreador campo. No instrumento aqui descrito, a latência de todo o assunto é
só que de Pd-se, cerca de 10 milissegundos (provavelmente não é difícil reduzi-la a 5 ou 6 usando
correções em tempo real do kernel, mas eu preferi usar off-the-shelf linux).}
\end{quotation}
...
execution of control level software.  \cite{rowe05}

\footnote{Sistemas de som interativos em primeiras implementações feitas geralmente uso do Musical
Instrument Digital Interface (MIDI) padrão. O padrão MIDI tem sido reconhecida desde o seu início
a ser lenta e limitada no seu âmbito de representação. A dependência de equipamento MIDI externo tem
condenado uma geração de trabalhos interativos para a obsolescência como o hardware necessário se torna indisponível.
Máquinas mais rápidas e mais baratas que nos últimos anos tornou possível a realização de análise, síntese,
de amostragem, e os efeitos sobre a CPU de um computador de uso geral pessoal, em simultâneo com o
execução de software nível de controle.}
\end{quotation}
...
de caso de desenvolvimento em puredata, podendo ser usado como estudo para implementação de outros algoritmos e idéias. \cite{navalha}
\end{quotation}
...
Segundo Silva \begin{quotation}
		...
		ser abordados tanto do ponto de vista da análise quanto da composição.
              \end{quotation} \cite{sampaio08:torno}.
...
\begin{figure}
\includegraphics[scale=.6]{sinc-randplay}
...
\begin{figure}
\includegraphics[scale=.6]{rand-editar}
...
\begin{figure}
\includegraphics[scale=.6]{sinc-controle-teclado}
...
módulos interativos embaixo, e um módulo de anĺaise de áudio na parte direita superior.

\begin{figure}
\includegraphics[scale=.4]{audio-midi}
...
\begin{figure}
\includegraphics[scale=.4]{sintese}
...
instrumentista aciona uma nota simultânea no sintetizador. É feito um cálculo da distância
...
\begin{figure}
\includegraphics[scale=.4]{sinc-midilive}
\caption{Visão da abstração [sinc-midilive]}
\label{sinc-midilive}
\end{figure}

\begin{figure}
\includegraphics[scale=.4]{sinc-harmonia}
...
\begin{figure}
\includegraphics[scale=.6]{trab-fomus}
...
O projeto Gemnotes\footnote{fonte: (http://sharktracks.co.uk/site/2010/12/gemnotes-progress/) escrito em 16/12/2010.} \cite{gemnotes}
é pioneiro no uso de Pd e Gem como motores de um sistema de notação
musical.
...
\begin{quotation}
This object is now complete, and the system is evolving to show beamed groups, ties, rests and chords. 
These are projected in realtime, and can be made to change at any time, on a computer screen.
\end{quotation} 
...
Outras soluções que já podem ser operadas em integração com SInCoPA podem ser apontadas para o futuro da pesquisa, como por exemplo:
